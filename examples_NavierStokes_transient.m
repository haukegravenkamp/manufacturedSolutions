...
g  = @(t) cos(pi*t).*exp(-t);                                               % time-dependent part of solution
dg = @(t) -pi*sin(pi*t).*exp(-t) - cos(pi*t).*exp(-t);                      % derivative wrt t

p = @(x,y) 100*x.^2;                                                % pressure

U = @(x,y,t) [u(x,y).*g(t); p(x,y)];            % collect all degrees of freedom
...
f = @(x,y,t) [fMom(x,y).*g(t) + u(x,y).*dg(t) ; fCon(x,y)];                                          % assemble in one vector
